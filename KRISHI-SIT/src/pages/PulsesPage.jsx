...
// Pulses have no category filter — we show all with a season badge instead
const SEASON_COLORS = {
    Kharif: { color: "#fcd34d", icon: "☀️" },
    Rabi: { color: "#a5f3fc", icon: "❄️" },
    Zaid: { color: "#86efac", icon: "🌱" },
};
const DEFAULT_SEASON = { color: "#34d399", icon: "🌿" };
...
// ── Pulse card — shows season + water info unique to pulses ──────────────────
const PulseCard = ({ item, onAdd, onIncrease, onDecrease, quantity, adding, updating, index }) => {
    const [hovered, setHovered] = useState(false);
    const season = SEASON_COLORS[item.season] || DEFAULT_SEASON;
    ...
    return (
        <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{ background: "rgba(255,255,255,0.04)", border: `1px solid ${hovered ? "rgba(52,211,153,0.25)" : "rgba(255,255,255,0.08)"}`, borderRadius: "20px", overflow: "hidden", display: "flex", flexDirection: "column", backdropFilter: "blur(16px)", boxShadow: hovered ? "0 20px 52px rgba(0,0,0,0.5)" : "0 6px 24px rgba(0,0,0,0.3)", transform: hovered ? "translateY(-6px) scale(1.01)" : "translateY(0) scale(1)", transition: "all 0.3s cubic-bezier(0.34,1.2,0.64,1)", animation: `cardIn 0.5s ${Math.min(index * 55, 500)}ms both ease`, position: "relative" }}>
            ...
            <div style={{ position: "relative", height: "180px", overflow: "hidden", background: "rgba(0,0,0,0.3)", flexShrink: 0 }}>
                <img src={`http://localhost:8003/images/${item.image}`} alt={item.name} style={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform 0.5s ease", transform: hovered ? "scale(1.07)" : "scale(1)" }} />
                <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(2,12,6,0.6) 0%, transparent 55%)" }} />

                {/* Season badge */}
                <div style={{ position: "absolute", top: "10px", left: "10px", display: "flex", alignItems: "center", gap: "5px", background: "rgba(2,12,6,0.75)", border: `1px solid ${season.color}40`, borderRadius: "99px", padding: "3px 10px", backdropFilter: "blur(8px)" }}>
                    <span style={{ fontSize: "11px" }}>{season.icon}</span>
                    <span style={{ fontSize: "10px", fontWeight: "700", color: season.color, letterSpacing: "0.5px" }}>{item.season || "All season"}</span>
                </div>
            ...
            <div style={{ padding: "16px 18px 18px", flex: 1, display: "flex", flexDirection: "column", gap: "4px" }}>
                ...
                {/* Extra pulse meta — season + water */}
                <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginTop: "4px" }}>
                    {item.season && (
                        <span style={{ fontSize: "10px", fontWeight: "600", color: season.color, background: `${season.color}18`, border: `1px solid ${season.color}30`, borderRadius: "99px", padding: "2px 8px" }}>
                            {season.icon} {item.season}
                        </span>
                    )}
                    {item.waterRequirement && (
                        <span style={{ fontSize: "10px", fontWeight: "600", color: "rgba(125,211,252,0.85)", background: "rgba(125,211,252,0.10)", border: "1px solid rgba(125,211,252,0.22)", borderRadius: "99px", padding: "2px 8px" }}>
                            💧 {item.waterRequirement}
                        </span>
                    )}
                </div>
...
const PulsesPage = () => {
    ...
    const { data, isLoading: loading, error } = useGetPulsesQuery();
    const [addToCart] = useAddToCartMutation();
    const [updateCartQuantity] = useUpdateCartQuantityMutation();
    const [removeFromCart] = useRemoveFromCartMutation();

    const handleAdd = async (pulse) => {
        setAddingId(pulse._id);
        try {
            await addToCart({ itemId: pulse._id.toString(), service: "pulse", name: pulse.name, price: pulse.price, image: pulse.image, category: pulse.category, quantity: 1 }).unwrap();
            setQuantities(q => ({ ...q, [pulse._id]: 1 }));
            toast.success(`${pulse.name} added to cart!`, { position: "bottom-right", autoClose: 2500, theme: "dark", transition: Bounce });
        } catch (err) {
            console.error(err);
    ...
    const handleIncrease = async (pulse) => {
        const prev = quantities[pulse._id] || 0;
        setQuantities(q => ({ ...q, [pulse._id]: prev + 1 }));
        setUpdatingId(pulse._id);
        try {
            await updateCartQuantity({ itemId: pulse._id.toString(), action: "inc" }).unwrap();
        } catch (err) {
            setQuantities(q => ({ ...q, [pulse._id]: prev }));
            toast.error("Couldn't update quantity", { position: "bottom-right", autoClose: 2000, theme: "dark", transition: Bounce });
        } finally { setUpdatingId(null); }
    };

    const handleDecrease = async (pulse) => {
        const current = quantities[pulse._id] || 0;
        if (current <= 0) return;
        const next = current - 1;
        setQuantities(q => ({ ...q, [pulse._id]: next }));
        setUpdatingId(pulse._id);
        try {
            if (next === 0) {
                await removeFromCart(pulse._id.toString()).unwrap();
                toast.info(`${pulse.name} removed from cart`, { position: "bottom-right", autoClose: 2000, theme: "dark", transition: Bounce });
            } else {
                await updateCartQuantity({ itemId: pulse._id.toString(), action: "dec" }).unwrap();
            }
        } catch (err) {
            setQuantities(q => ({ ...q, [pulse._id]: current }));
            toast.error("Couldn't update quantity", { position: "bottom-right", autoClose: 2000, theme: "dark", transition: Bounce });
        } finally { setUpdatingId(null); }
    };
        ...
        <>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "28px", flexWrap: "wrap", gap: "14px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    ...
                    <span style={{ fontSize: "11px", fontWeight: "700", letterSpacing: "2.5px", textTransform: "uppercase", color: "rgba(52,211,153,0.55)" }}>All Pulses & Legumes</span>
                </div>
                {data && <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.32)", fontWeight: "600", animation: "fadeIn 0.3s both ease" }}>{data.length} {data.length === 1 ? "pulse" : "pulses"} found</div>}
            </div>

            {loading && <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: "16px" }}>{[...Array(8)].map((_, i) => <div key={i} style={{ height: "300px", borderRadius: "20px", background: "rgba(255,255,255,0.04)", animation: `skeletonPulse 1.4s ${i * 100}ms ease-in-out infinite` }} />)}</div>}

            {error && <div style={{ textAlign: "center", padding: "60px 24px", background: "rgba(239,68,68,0.07)", border: "1px solid rgba(239,68,68,0.18)", borderRadius: "20px" }}><div style={{ fontSize: "40px", marginBottom: "14px" }}>⚠️</div><div style={{ fontSize: "15px", fontWeight: "700", color: "#f87171" }}>Failed to load pulses</div><div style={{ fontSize: "13px", color: "rgba(255,255,255,0.3)", marginTop: "6px" }}>Please check your connection and try again.</div></div>}

            {!loading && !error && data?.length === 0 && <div style={{ textAlign: "center", padding: "60px 24px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "20px" }}><div style={{ fontSize: "48px", marginBottom: "14px" }}>🫘</div><div style={{ fontSize: "16px", fontWeight: "700", color: "rgba(255,255,255,0.4)" }}>No pulses found</div></div>}

            {!loading && !error && data?.length > 0 && (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: "16px" }}>
                    {data.map((pulse, i) => (
                        <PulseCard key={pulse._id} item={pulse} index={i}
                            quantity={quantities[pulse._id] || 0}
                            onAdd={() => handleAdd(pulse)}
                            onIncrease={() => handleIncrease(pulse)}
                            onDecrease={() => handleDecrease(pulse)}
                            adding={addingId === pulse._id}
                            updating={updatingId === pulse._id}
                        />
                    ))}
                </div>
            )}
