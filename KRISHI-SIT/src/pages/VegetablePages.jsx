...
const CATEGORIES = [
    ...
    { key: "Leafy Vegetable", icon: "🌿", color: "#86efac" },
    { key: "Root Vegetable", icon: "🥕", color: "#fdba74" },
    { key: "Bulb Vegetable", icon: "🧅", color: "#fcd34d" },
    { key: "Fruit Vegetable", icon: "🍅", color: "#fca5a5" },
    { key: "Cabbage Family", icon: "🥦", color: "#6ee7b7" },
    { key: "Pod & Seed Vegetable", icon: "🫘", color: "#a5f3fc" },
    { key: "Regional/Exotic", icon: "🌶️", color: "#f9a8d4" },
];
        ...
        return (
            <button
                ...
                style={{
                    ...
                    background: busy ? "rgba(52,211,153,0.12)" : hovered ? "linear-gradient(135deg,#059669,#047857)" : "rgba(52,211,153,0.10)",
            ...
            >
                {busy ? (
                    <><span style={{ display: "inline-block", width: "14px", height: "14px", border: "2px solid rgba(52,211,153,0.3)", borderTopColor: "#34d399", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />Adding…</>
                ) : <>🛒 Add to Cart</>}
            </button>
        );
    }
...
// ── Vegetable card ────────────────────────────────────────────────────────────
const VegCard = ({ item, imageBase, serviceKey, onAdd, onIncrease, onDecrease, quantity, adding, updating, index }) => {
    ...
    return (
        <div
            ...
            style={{
                ...
                animation: `cardIn 0.5s ${Math.min(index * 55, 500)}ms both ease`,
                position: "relative",
            }}
        >
            ...
            <div style={{ position: "relative", height: "180px", overflow: "hidden", background: "rgba(0,0,0,0.3)", flexShrink: 0 }}>
                <img src={`${imageBase}/images/${item.image}`} alt={item.name} style={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform 0.5s ease", transform: hovered ? "scale(1.07)" : "scale(1)" }} />
...
const VegetablesPage = () => {
    ...
    const allData = useGetVegetablesQuery(undefined, { skip: selectedCategory !== "All" });
    const categoryData = useGetVegetablesByCategoryQuery(selectedCategory, { skip: selectedCategory === "All" });
    ...
    const handleAdd = async (veg) => {
        setAddingId(veg._id);
        try {
            await addToCart({ itemId: veg._id.toString(), service: "vegetable", name: veg.name, price: veg.price, image: veg.image, category: veg.category, quantity: 1 }).unwrap();
            setQuantities(q => ({ ...q, [veg._id]: 1 }));
            toast.success(`${veg.name} added to cart!`, { position: "bottom-right", autoClose: 2500, theme: "dark", transition: Bounce });
        } catch (err) {
            console.error(err);
    ...
    const handleIncrease = async (veg) => {
        const prev = quantities[veg._id] || 0;
        setQuantities(q => ({ ...q, [veg._id]: prev + 1 }));
        setUpdatingId(veg._id);
        try {
            await updateCartQuantity({ itemId: veg._id.toString(), action: "inc" }).unwrap();
        } catch (err) {
            setQuantities(q => ({ ...q, [veg._id]: prev }));
            toast.error("Couldn't update quantity", { position: "bottom-right", autoClose: 2000, theme: "dark", transition: Bounce });
        } finally { setUpdatingId(null); }
    };

    const handleDecrease = async (veg) => {
        const current = quantities[veg._id] || 0;
        if (current <= 0) return;
        const next = current - 1;
        setQuantities(q => ({ ...q, [veg._id]: next }));
        setUpdatingId(veg._id);
        try {
            if (next === 0) {
                await removeFromCart(veg._id.toString()).unwrap();
                toast.info(`${veg.name} removed from cart`, { position: "bottom-right", autoClose: 2000, theme: "dark", transition: Bounce });
            } else {
                await updateCartQuantity({ itemId: veg._id.toString(), action: "dec" }).unwrap();
            }
        } catch (err) {
            setQuantities(q => ({ ...q, [veg._id]: current }));
            toast.error("Couldn't update quantity", { position: "bottom-right", autoClose: 2000, theme: "dark", transition: Bounce });
        } finally { setUpdatingId(null); }
    };
    ...
    return (
        <>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "28px", flexWrap: "wrap", gap: "14px" }}>
                ...
                {data && <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.32)", fontWeight: "600", animation: "fadeIn 0.3s both ease" }}>{data.length} {data.length === 1 ? "vegetable" : "vegetables"} found</div>}
            </div>

            {loading && (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: "16px" }}>
                    {[...Array(8)].map((_, i) => <div key={i} style={{ height: "280px", borderRadius: "20px", background: "rgba(255,255,255,0.04)", animation: `skeletonPulse 1.4s ${i * 100}ms ease-in-out infinite` }} />)}
                </div>
            )}
            ...
            {error && (
                <div style={{ textAlign: "center", padding: "60px 24px", background: "rgba(239,68,68,0.07)", border: "1px solid rgba(239,68,68,0.18)", borderRadius: "20px" }}>
                    ...
                    <div style={{ fontSize: "15px", fontWeight: "700", color: "#f87171" }}>Failed to load vegetables</div>
            ...
            {!loading && !error && data?.length === 0 && (
                <div style={{ textAlign: "center", padding: "60px 24px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "20px" }}>
                    ...
                    <div style={{ fontSize: "16px", fontWeight: "700", color: "rgba(255,255,255,0.4)" }}>No vegetables in this category</div>
                </div>
            )}
            ...
            {!loading && !error && data?.length > 0 && (
                <div key={selectedCategory} style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: "16px" }}>
                    {data.map((veg, i) => (
                        <VegCard key={veg._id} item={veg} imageBase="http://localhost:8001" serviceKey="vegetable" index={i}
                            quantity={quantities[veg._id] || 0}
                            onAdd={() => handleAdd(veg)}
                            onIncrease={() => handleIncrease(veg)}
                            onDecrease={() => handleDecrease(veg)}
                            adding={addingId === veg._id}
                            updating={updatingId === veg._id}
                        />
                    ))}
                </div>
            )}
